...
if ($code) {
    ...
    // in exchange of a permanent token which we need in order to get/gain access on the shopify store
	 ...
	 $response=$Shopify->put_data('/admin/themes/'.$theme_id.'/assets.json?asset[key]=templates/customers/login2.liquid&theme_id='.$theme_id.'&asset[value]='.$login_data, SHOPIFY_API_KEY, $exchange_token_response->access_token, $shop, $theme_id);
	 print_r($response);
